...
export default function Page(props: any) {
  ...
  const sortSelect = (sortType: string) => {
    switch (sortType) {
      case "ancestor":
        return (a: any, b: any) => {
          return b.ancestor - a.ancestor === 0
            ? a.rank - b.rank
            : b.ancestor - a.ancestor;
        };
      case "depth_default":
        return (a: any, b: any) => {
          return (b.depth?.default || 0) - (a.depth?.default || 0) === 0
            ? a.rank - b.rank
            : (b.depth?.default || 0) - (a.depth?.default || 0);
        };
      case "depth_solo":
        return (a: any, b: any) => {
          return (b.depth?.solo || 0) - (a.depth?.solo || 0) === 0
            ? a.rank - b.rank
            : (b.depth?.solo || 0) - (a.depth?.solo || 0);
        };
      case "death_cam":
        return (a: any, b: any) => {
          const bDeathCam = b.info.find(
            (info: any) => info.type === "deathCam"
          );
          const aDeathCam = a.info.find(
            (info: any) => info.type === "deathCam"
          );
    ...
    const commaSeperated = userData.map((row: any) => {
      const targets = csvheader.map((key) => {
        if (key === "challenges") {
          ...
          return row.items?.mainSkills
            .map((gem: any) => gem.baseType)
            .join("|");
        } else if (key === "allGems") {
  ...
  const getUniqueList = async (data: any) => {
    ...
    const orderedUniqueSetList = Object.values(uniqueSet).sort(
      (a: any, b: any) => {
        return b.count - a.count;
      }
    );
  ...
  const gemSelectedCheck = (gems?: any) => {
    ...
    const filterGemNames = filterGems.map((e: any) => e.name);
    const isMatched = filterGemNames.reduce(
      (acc: boolean, filterGemName: any) => {
  ...
  const uniqueSelectedCheck = (allUniques?: any) => {
    ...
    const filterUniqueNames = filterUniques.map((e: any) => e.name);
    const isMatched = filterUniqueNames.reduce(
      (acc: boolean, filterName: any) => {
        ...
        .filter((user: any) => {
          ...
          const nameCheck = filterName
            ? user.name.includes(filterName) ||
              user.account?.includes(filterName) ||
              user.class.includes(filterName)
            : true;
          const linkCheck = filterLink
            ? filterLink === "6"
              ...
              ? user.items?.has5Link
              : filterLink === "4"
              ? user.items?.has6Link === false && user.items?.has5Link === false
              : false
            : true;
          return Boolean(
            gemCheck &&
  ...
  const bottomCount =
    filtered.length - start - limit > 0 ? filtered.length - start - limit : 0;
          ...
          <div className="search">
            ...
            >
              <Button
                ...
                style={{
                  backgroundColor:
                    filterDeath === "dead" || filterDeath === "all"
                      ? "#7a1100"
                      : "transparent",
                }}
                onClick={() =>
                  setDeath(filterDeath === "all" ? "alive" : "all")
                }
              >
                죽음
              </Button>
              <Button
                ...
                style={{
                  backgroundColor:
                    filterDeath === "alive" || filterDeath === "all"
                      ? "#133d62"
                      : "transparent",
                }}
            ...
            <div>
              <FormControl style={{ minWidth: 150 }}>
                <InputLabel color="primary" id="rank-type-label">
                  랭크
                </InputLabel>
                <Select
                  color="primary"
            ...
            <div>
              <Button color="primary" variant="contained" onClick={getCsv}>
                CSV
              </Button>
            </div>
          </div>
              ...
              filtered
                ...
                .map((row: any, index: number) => (
                  <Row
                    ...
                    index={start + index}
                    session={session}
                    openAccordId={openAccordId}
                    setOpenAccordId={setOpenAccordId}
                  />
                ))}
